...
function geoToSvg(lat: number, lng: number): { x: number; y: number } {
  ...
  const x =
    svgMinX + ((lng - minLng) / (maxLng - minLng)) * (svgMaxX - svgMinX);
  const y =
    svgMaxY - ((lat - minLat) / (maxLat - minLat)) * (svgMaxY - svgMinY);
  return { x: Math.round(x), y: Math.round(y) };
}

// State capitals with real geographic coordinates and display labels
const stateCapitals: Record<
  string,
  {
    lat: number;
    lng: number;
...
> = {
  ...
  adamawa: { lat: 9.33, lng: 12.4, label: 'Yola' },
  'akwa-ibom': {
    lat: 5.01,
    lng: 7.85,
    label: 'Uyo',
  ...
  kano: { lat: 12.0, lng: 8.52, label: 'Kano' },
  katsina: { lat: 13.01, lng: 7.6, label: 'Katsina' },
  kebbi: {
    lat: 12.45,
    lng: 4.2,
    label: 'B. Kebbi',
  ...
  kogi: { lat: 7.8, lng: 6.73, label: 'Lokoja', labelOffset: [-8, 14] },
  kwara: {
    lat: 8.5,
    lng: 4.55,
    label: 'Ilorin',
  ...
  ondo: { lat: 7.25, lng: 5.2, label: 'Akure', labelOffset: [8, 10] },
  osun: {
    lat: 7.77,
    lng: 4.56,
    label: 'Osogbo',
